...
//Method that sets up the vertices that build a single SphereModel
void SphereModel::setupVC()
{
...
//SphereModel constructor that passes a vec3 as the original position
SphereModel::SphereModel(glm::vec3 origin_a)
{
	setupVC();
	origin = origin_a;
...
void SphereModel::setVertexController(bool position, bool texture, bool color)
{
...
//Method that returns the vertices of the SphereModel 
Vertex SphereModel::getSampleVertex()
{
	return vc->vertices.front();
}

//Method that applies a transformation matrix to the vertices of the SphereModel
void SphereModel::transform(glm::mat4 transmat)
{
	vc->transform(transmat);
}

//Method that translates the SphereModel from origin (used for when moving the model so it is according to the origin of the model)
void SphereModel::translate_fromOrigin()
{
